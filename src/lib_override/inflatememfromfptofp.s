...
;
; unsigned __fastcall__ inflatememfromfptofp (unsigned char* dest, const unsigned char* source, FILE *fp_input , FILE *fp_output);
;
...
;
        ...
        .export         _inflatememfromfptofp

        .import         incsp2, popax
        .importzp       sp, sreg, ptr1, ptr2, ptr3, ptr4, tmp1, tmp2, tmp3, tmp4
...
fp_input                    :=  tmp1 ; and tmp1
fp_output                   :=  tmp3 ; and tmp3
...
_inflatememfromfptofp:
     ;   jmp     _inflatememfromfptofp
        ; A & X contains fp_output
        ; FILE *fp_output);
        sta     fp_output
        stx     fp_output + 1
...
;       clc
        adc     outputPointer+1
        sta     inflateCodes_sourcePointer + 1 ; $5CFB
        jsr     copyByte
        jsr     copyByte
...
getBit:
        ...
        ldy     #$00 ; Read 1 byte dont change it Y must be equal to 0 for next instruction
        ; Y must be equal to 0
        lda     (fp_input),y
        tax
        lda     #$01 ; Read 1 byte
        BRK_TELEMON XFREAD
        ; inputPointer contains one byte
        pla
        tax
...
copyByte:
        ;$4872
        ldy     outputPointer ; $ba val  = $25
        lda     (inflateCodes_sourcePointer),y ; $bc val $5cfb avec Y = $5D20 $5D1D
        ldy     #0
; Write a byte
storeByte:
        ; La 1ère itération vient d'un jsr de storebyte et non pas de copybyu
        ; $4878
;       ldy     #0
      ;  jmp     storeByte
        sta     uncompressedData

        sta     (outputPointer),y ; 1ère itération Y = 0 $5CE4
